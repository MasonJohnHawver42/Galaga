...
#include "bond/entt/meta_helper.hpp"
#include <set>
#include <iostream>
...
template <typename Component> void register_meta_component() {
  ...
  entt::meta<Component>()
    .type(entt::type_hash<Component>::value())
...
sol::table open_registry(sol::this_state s) {
    ...
    entt_module.new_usertype<entt::registry>("registry",
        ...
        "size",    [](entt::registry &self) { return self.storage<entt::entity>().size(); },
        "alive",   [](entt::registry &self) { return self.storage<entt::entity>().in_use(); },
        ...
        "any_of", [](const sol::table &self, entt::entity entity, const sol::variadic_args &va) {
            ...
            return std::any_of(types.cbegin(), types.cend(),
                [&](auto type_id) { return has(self, entity, type_id); }
            );
        },
        
        "runtime_view", [](entt::registry &self, const sol::variadic_args &va) {
            const auto types = collect_types(va); // std::set<entt::id_type>
            entt::runtime_view view{};
            for (auto type_id : types) 
            {
                auto* storage_ptr = self.storage(type_id);
